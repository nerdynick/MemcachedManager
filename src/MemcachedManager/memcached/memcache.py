...
class Client(local):
    ...
    def _get(self, cmd, key, unpickel=True):
        self.check_key(key)
        server, key = self._get_server(key)
        if not server:
            return None
    ...
    def get(self, key, unpickel=True):
        '''Retrieves a key from the memcache.

        @return: The value or None.
        '''
        return self._get('get', key, unpickel=unpickel)
    ...
    def get_multi(self, keys, key_prefix='', unpickel=True):
        '''
        ...
        for server in server_keys.iterkeys():
            try:
                ...
                while line and line != 'END':
                    ...
                    if rkey is not None:
                        val = self._recv_value(server, flags, rlen, unpickel=unpickel)
    ...
    def _recv_value(self, server, flags, rlen, unpickel=True):
        rlen += 2 # include \r\n
        ...
        if  flags == 0 or flags == Client._FLAG_COMPRESSED:
            ...
            val = long(buf)
        elif flags & Client._FLAG_PICKLE and unpickel is True:
            try:
                file = StringIO(buf)
